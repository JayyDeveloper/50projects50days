...
const inputCls = "w-full px-3 py-2.5 rounded-xl border border-white/30 dark:border-white/10 bg-white/40 dark:bg-white/5 backdrop-blur-sm text-sm outline-none focus:ring-2 focus:ring-teal-300 transition"
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div>
        ...
        <div className="mb-2 h-2 bg-white/20 dark:bg-white/10 rounded-full overflow-hidden">
          ...
          {Object.entries(COLORS).map(([key, c]) => (
            <button key={key} type="button" onClick={() => setColor(key)}
              className={`w-8 h-8 rounded-full bg-gradient-to-br ${c.grad} transition-all hover:scale-110 ${
                color === key ? 'ring-2 ring-offset-2 ring-gray-400 dark:ring-offset-black/80 scale-110' : ''
              }`}
            />
          ))}
        </div>
  ...
  return (
    <div className="flex items-center gap-3 group py-2.5 border-b border-white/10 dark:border-white/8 last:border-0 hover:bg-white/20 dark:hover:bg-white/10 rounded-xl px-2 -mx-2 transition-colors">
      ...
      <div className="opacity-0 group-hover:opacity-100 flex gap-0.5 transition-opacity flex-shrink-0">
        <button onClick={onEdit} className="p-1 rounded-lg hover:bg-white/20 dark:hover:bg-white/10 transition-colors"><Pencil size={10} className="text-gray-400" /></button>
    ...
    <div className="bg-white/50 dark:bg-white/5 backdrop-blur-2xl rounded-3xl border border-white/30 dark:border-white/10 shadow-2xl shadow-black/10 p-6">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-teal-100/80 to-emerald-100/80 dark:from-teal-500/20 dark:to-emerald-500/20 flex items-center justify-center">
